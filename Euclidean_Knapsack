...
# Knapsack algorithm using Euclidean distance method
knapsack_euclidean <- function(weights, values, capacity) {
  n <- length(weights)
...
main <- function() {
  ...
  start_time <- Sys.time()
  result <- knapsack_euclidean(knapsack_data$weights, knapsack_data$values, capacity)
  end_time <- Sys.time()
